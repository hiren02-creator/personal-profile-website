...
Set the Vercel Root Directory to `frontend`. The `frontend/vercel.json` configuration selects Vite, installs dependencies with `npm install --include=dev`, builds with `npm run build`, and publishes `dist` relative to that root. Vite and its React plugin are correctly declared as development dependencies; they must be installed during the build.

Set `VITE_SUPABASE_URL` and `VITE_SUPABASE_ANON_KEY` in Vercel before building. Use only the public client key, never a secret or service-role key. Local values belong in the ignored `.env.local` file; `.env.example` contains placeholders only.

To verify locally, run from `frontend`:
